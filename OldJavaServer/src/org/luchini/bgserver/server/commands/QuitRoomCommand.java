...
public class QuitRoomCommand implements ServerCommand {
	...
	public AbstractResponse exec(Protocol protocol, BGServerThread thread,
		...
		if (protocol.getServerEngine().quitRoom(thread.getPlayerInfo(), params[0])) {
			out = new BasicResponse(this.command(), BasicResponse.OK);
			CommandUtils.unhookGameListeners(protocol.getServerEngine(), thread, params[0]);
		}
		return out;	
	}
