...
public class EnterRoomCommand implements ServerCommand {
	...
	public AbstractResponse exec(Protocol protocol, BGServerThread thread,
		...
		if (protocol.getServerEngine().enterRoom(thread.getPlayerInfo(), params[0])) {
			out = new BasicResponse(this.command(), BasicResponse.OK);
			CommandUtils.hookGameListeners(protocol.getServerEngine(), thread, params[0]);
